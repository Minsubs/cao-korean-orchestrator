"""Kiro CLI (``kiro_cli``) provider adapter — npm install only.

Kiro CLI has no non-interactive extension/MCP management surface this adapter
could drive, so v1 is deliberately install-only: the sole supported action is
the target-exempt ``install_cli`` (a fixed, server-constant npm package —
never a client-supplied name). Every other action — list, search, install,
remove, update, update_all — is refused with an actionable reason.
"""
...
# ``{action: "install_cli", provider: "kiro_cli"}`` and the ``target`` field
# (if any) is ignored by :meth:`KiroCliAdapter.plan`.
_CLI_PACKAGE = "@anthropic-ai/kiro-cli"

_INSTALL_ONLY_REASON = "kiro_cli는 CLI 설치만 지원해요 — 확장 관리는 터미널에서 하세요"


class KiroCliAdapter(ExtensionAdapter):
    """Install-only ``kiro_cli`` adapter: npm-bootstraps the CLI, nothing else."""
    ...
    def detect(self) -> AdapterEnv:
        """Locate ``kiro-cli`` with ``which`` only (never executes it)."""
