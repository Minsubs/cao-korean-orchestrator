"""Copilot CLI (``copilot_cli``) provider adapter — npm install only.

Copilot CLI has no non-interactive extension/MCP management surface this
...
# ``{action: "install_cli", provider: "copilot_cli"}`` and the ``target`` field
# (if any) is ignored by :meth:`CopilotCliAdapter.plan`.
_CLI_PACKAGE = "@github/copilot"

_INSTALL_ONLY_REASON = "copilot_cli는 CLI 설치만 지원해요 — 확장 관리는 터미널에서 하세요"


class CopilotCliAdapter(ExtensionAdapter):
    """Install-only ``copilot_cli`` adapter: npm-bootstraps the CLI, nothing else."""
    ...
    def detect(self) -> AdapterEnv:
        """Locate ``copilot`` with ``which`` only (never executes it)."""
