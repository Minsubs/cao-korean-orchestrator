...
def test_ignores_unused_provider_and_reports_installed_version_error(monkeypatch, tmp_path):
    ...
    assert "provider_missing" not in by_code
...
def test_expected_packaged_profile_mirrors_are_not_warnings(monkeypatch, tmp_path):
    _neuter_skills(monkeypatch, tmp_path)
    monkeypatch.setattr(diagnostics, "list_providers", lambda: [])
    monkeypatch.setattr(
        diagnostics,
