...
def test_probe_uses_the_resolved_path_not_the_bare_binary(monkeypatch):
    """argv[0] is the resolved path so execvp does not re-walk the PATH."""
    run_spy = MagicMock(return_value=probe.ProbeResult(0, "claude 2.1.211\n", "", False))
    monkeypatch.setattr(providers.cache, "cached_which", lambda binary: f"/usr/bin/{binary}")
    monkeypatch.setattr(providers.probe, "run", run_spy)

    providers.list_providers(use_cache=False)

    argv0s = {call.args[0][0] for call in run_spy.call_args_list}
    assert argv0s and all(argv0.startswith("/usr/bin/") for argv0 in argv0s)
